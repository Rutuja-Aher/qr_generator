...
class QR_Generator:
 ...
 #====Employee DETAILS window=======
  ...
  self.lbl_msg=Label(emp_Frame,text=self.msg,font=("times new roman",20),bg='white',fg='purple')
...
#====Employee QR code window=======
 ...
 def generate(self):
    if self.var_designation.get()==''or self.var_emp_code.get()=='' or self.var_name.get()=='' or self.var_department.get()=='':
        ...
        qr_data=(f"Empolyee ID:{self.var_emp_code.get()}\nEmployee Name:{self.var_name.get()}\nDepartment:{self.var_department.get()}\nDesignation:{self.var_designation.get()}")
        qr_code=qrcode.make(qr_data)
        qr_code=resizeimage.resize_cover(qr_code,[180,180])
        print(qr_code)
        qr_code.save("Employee"+str(self.var_emp_code.get())+'.png')
